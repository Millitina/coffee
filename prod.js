...
async function loadProductData() {
    try {
        // Получаем имя продукта из URL (например, /product/cappuccino)
        ...
        // Отправляем запрос на сервер для получения данных продукта
        ...
        // Сохраняем catalogId в глобальную переменную
        window.productCatalogId = productData.id;

        // Обновляем страницу полученными данными
...
function getCatalogId() {
    // Используем ID из глобальной переменной, полученный от сервера
    if (window.productCatalogId) {
        return window.productCatalogId;
    }

    // Фолбэк: пытаемся получить из атрибутов
