        ...
        html += `
            <div class="product-item" data-id="${product.id}" data-price="${product.cost}" data-rating="${product.stars || 0}" data-size="${getSizeFromSizes(product.sizes)}">
                ...
                <div class="product-info">
                    <a href="http://localhost:3000/product/${encodedName}" class="product-name-link">
...
function initProductTooltips() {
    ...
    // Создаем структуру тултипа
    tooltip.innerHTML = `
        <div class="tooltip-content">
            <div class="tooltip-header">
                <span class="tooltip-name" id="tooltipName"></span>
                <div class="tooltip-rating" id="tooltipRating">
                    <div class="stars-container"></div>
                    <span class="rating-number"></span>
                </div>
            </div>
            <div class="tooltip-sizes">
                <span class="tooltip-label">Размеры:</span>
                <div class="sizes-container" id="tooltipSizes"></div>
            </div>
            <div class="tooltip-ingredients">
                <span class="tooltip-label">Состав:</span>
                <ul class="ingredients-list" id="tooltipIngredients"></ul>
            </div>
        </div>
    `;
    ...
    const hasJQuery = typeof $ !== 'undefined' && $.ajax;
    ...
    function makeAjaxRequest(url, options = {}) {
        return new Promise((resolve, reject) => {
            ...
            // Если есть jQuery, используем его AJAX
            if (hasJQuery) {
                console.log(`[AJAX] Используем jQuery для: ${url}`);
                $.ajax({
                    url: url,
                    ...
                    error: function(xhr, status, error) {
                        reject(new Error(error || status || 'Ошибка запроса'));
                    }
                });
            } 
            // Иначе используем нативный fetch
            else {
                console.log(`[AJAX] Используем fetch для: ${url}`);
    ...
    // Функция для получения данных о товаре с кэшированием
    async function fetchProductInfo(productName) {
    // Проверяем кэш
    if (cache.has(productName)) {
        console.log(`[КЭШ] Данные для: ${productName}`);
        return cache.get(productName);
    }
    
    // Проверяем, есть ли уже активный запрос
    if (pendingRequests.has(productName)) {
        console.log(`[ОЖИДАНИЕ] Запрос для: ${productName}`);
        return pendingRequests.get(productName);
    }
    ...
    const requestPromise = (async () => {
        try {
            ...
            const url = `${API_BASE_URL}/catalog/short/info?name=${encodedName}`;
            
            console.log(`[AJAX ЗАПРОС] Отправка к: ${url}`);
            
            // Используем AJAX запрос
            const data = await makeAjaxRequest(url, { timeout: 3000 });
            
            // Исправленная обработка размеров
            let sizesArray = [];
            
            // Проверяем, что sizes существует и не пустой
            if (data.sizes && Array.isArray(data.sizes) && data.sizes.length > 0) {
                // Если это массив строк, используем как есть
                if (typeof data.sizes[0] === 'string') {
                    sizesArray = data.sizes;
                } 
                // Если это массив объектов с полем size
                else if (typeof data.sizes[0] === 'object' && data.sizes[0].size) {
                    sizesArray = data.sizes.map(item => item.size);
                }
                // Если это строка с размерами через запятую
                else if (typeof data.sizes === 'string') {
                    sizesArray = data.sizes.split(',').map(s => s.trim());
                }
            }
            
            // Если размеры не получены, пытаемся получить из базы данных через другой эндпоинт
            if (sizesArray.length === 0) {
                try {
                    // Пробуем получить полную информацию о продукте
                    const fullInfoUrl = `${API_BASE_URL}/catalog/product?name=${encodedName}`;
                    const fullData = await makeAjaxRequest(fullInfoUrl, { timeout: 2000 });
                    
                    if (fullData && fullData.sizes) {
                        if (Array.isArray(fullData.sizes)) {
                            sizesArray = fullData.sizes;
                        } else if (typeof fullData.sizes === 'string') {
                            sizesArray = fullData.sizes.split(',').map(s => s.trim());
                        }
                    }
                } catch (err) {
                    console.log(`Не удалось получить полную информацию для ${productName}`);
                }
            }
            
            // Если все еще нет размеров, используем только те, которые есть в базе
            // НЕ добавляем S, M, L по умолчанию
            if (sizesArray.length === 0) {
                // Пытаемся определить размер из контекста товара
                const productCard = document.querySelector(`.product-item[data-id="${data.id || ''}"]`);
                if (productCard && productCard.dataset.size) {
                    sizesArray = [productCard.dataset.size];
                } else {
                    sizesArray = ['Информация отсутствует'];
                }
            }
            ...
            const formattedData = {
                ...
                sizes: sizesArray,
                ingredients: data.ingredients || ['Информация отсутствует']
            };
            
            // Сохраняем в кэш на 5 минут
            cache.set(productName, formattedData);
            
            // Автоматическое удаление из кэша через 5 минут
            setTimeout(() => {
                cache.delete(productName);
                console.log(`[КЭШ] Удалены данные для: ${productName}`);
            }, 300000);
            
            console.log(`[УСПЕХ] Загружены данные для: ${productName}`, formattedData);
            return formattedData;
            
        } catch (error) {
            console.error(`[ОШИБКА] Загрузка для ${productName}:`, error.message);
            const defaultData = getDefaultProductData(productName);
            cache.set(productName, defaultData);
            return defaultData;
        } finally {
            pendingRequests.delete(productName);
        }
    })();
    
    pendingRequests.set(productName, requestPromise);
    return requestPromise;
}
    
    // Дефолтные данные (быстрые)
    function getDefaultProductData(productName) {
    const name = productName.toLowerCase();
    let defaultSizes = [];
    let defaultIngredients = ['Информация уточняется'];
    let defaultRating = 4.0;
    
    // Определяем данные по категориям
    if (name.includes('кофе') || name.includes('латте') || name.includes('капучино') || name.includes('американо')) {
        defaultIngredients = ['Кофе в зернах', 'Вода', 'Молоко (опционально)'];
        defaultRating = 4.5;
        defaultSizes = ['S', 'M', 'L'];
    } else if (name.includes('чай') || name.includes('tea')) {
        defaultIngredients = ['Чайные листья', 'Вода', 'Сахар (по желанию)'];
        defaultRating = 4.3;
        defaultSizes = ['S', 'M', 'L'];
    } else if (name.includes('круассан') || name.includes('croissant')) {
        defaultIngredients = ['Мука пшеничная', 'Масло сливочное', 'Дрожжи', 'Сахар', 'Соль'];
        defaultRating = 4.7;
        defaultSizes = ['S'];
    } else if (name.includes('пирож') || name.includes('булочка')) {
        defaultIngredients = ['Мука', 'Яйца', 'Сахар', 'Начинка'];
        defaultRating = 4.4;
        defaultSizes = ['S', 'M'];
    } else if (name.includes('сок') || name.includes('juice')) {
        defaultIngredients = ['Свежие фрукты', 'Вода'];
        defaultRating = 4.2;
        defaultSizes = ['S', 'M', 'L'];
    } else if (name.includes('торт') || name.includes('cake')) {
        defaultIngredients = ['Мука', 'Яйца', 'Сахар', 'Крем'];
        defaultRating = 4.8;
        defaultSizes = ['S', 'M', 'L'];
    } else {
        // Для остальных товаров не добавляем размеры по умолчанию
        defaultSizes = ['Информация отсутствует'];
    }
    ...
    function updateTooltipContent(data) {
        ...
        const sizesContainer = document.getElementById('tooltipSizes');
        const ingredientsList = document.getElementById('tooltipIngredients');
        ...
        // Обновляем рейтинг
        if (starsContainer && ratingNumber) {
            const rating = Math.min(5, Math.max(0, data.rating));
            ...
            ratingNumber.textContent = data.rating > 0 ? `(${data.rating.toFixed(1)})` : '';
        }
        
        // Обновляем размеры
        if (sizesContainer && data.sizes) {
            const sizesHtml = data.sizes.map(size => {
                const sizeClass = size === 'S' ? 'small' : (size === 'M' ? 'medium' : 'large');
                return `<span class="size-badge-tooltip ${sizeClass}">${size}</span>`;
            }).join('');
            sizesContainer.innerHTML = sizesHtml || '<span class="no-data">—</span>';
        }
        ...
        if (ingredientsList && data.ingredients) {
            const ingredientsHtml = data.ingredients
                ...
                .join('');
            ingredientsList.innerHTML = ingredientsHtml || '<li>Информация отсутствует</li>';
        }
    }
    ...
    function positionTooltip(element, event) {
        ...
        let left = rect.right + 15 + scrollLeft;
        let top = rect.top + scrollTop;
        
        if (left + tooltipRect.width > window.innerWidth + scrollLeft) {
            left = rect.left - tooltipRect.width - 15 + scrollLeft;
        }
    ...
    // Показ тултипа
    async function showTooltip(element, productName, event) {
        ...
        // Проверяем кэш
        let cachedData = cache.get(productName);
        if (cachedData) {
        ...
        if (currentProductName === productName && tooltip.classList.contains('visible')) {
        ...
        productItems.forEach(item => {
            ...
            productCard.addEventListener('mouseenter', function(e) {
                ...
                activeTimeout = setTimeout(() => {
                    showTooltip(this, productName, e);
                }, 150);
            });
            
            productCard.addEventListener('mousemove', function(e) {
                if (tooltip.classList.contains('visible') && currentProductName === productName) {
                    positionTooltip(this, e);
                }
            });
    ...
    // Наблюдатель за новыми товарами
    ...
    console.log(`[ИНИЦИАЛИЗАЦИЯ] Тултипы готовы, jQuery ${hasJQuery ? 'доступен' : 'недоступен'}`);
}

// Функция предзагрузки данных с AJAX
async function preloadProductData() {
    const productItems = document.querySelectorAll('.product-item');
    const productNames = Array.from(productItems)
    ...
    console.log(`[ПРЕДЗАГРУЗКА] Начинаем загрузку для ${uniqueNames.length} товаров`);
    
    // Загружаем данные для первых 5 товаров
    const toPreload = uniqueNames.slice(0, 5);
    
    // Проверяем наличие jQuery
    const hasJQuery = typeof $ !== 'undefined' && $.ajax;
    
    // Функция AJAX запроса для предзагрузки
    function preloadAjax(url) {
        return new Promise((resolve, reject) => {
            if (hasJQuery) {
                $.ajax({
                    url: url,
                    ...
                    timeout: 5000,
                    success: function(data) {
                        console.log(`[ПРЕДЗАГРУЗКА] Успешно: ${url}`);
                        resolve(data);
                    },
                    error: function(xhr, status, error) {
                        console.log(`[ПРЕДЗАГРУЗКА] Ошибка: ${url} - ${error}`);
                        reject(error);
                    }
                });
            } else {
                ...
                .catch(error => {
                    console.log(`[ПРЕДЗАГРУЗКА] Ошибка: ${url} - ${error.message}`);
                    reject(error);
                });
            }
        });
    ...
    const promises = toPreload.map(name => {
        const encodedName = encodeURIComponent(name);
        const url = `${API_BASE_URL}/catalog/short/info?name=${encodedName}`;
        return preloadAjax(url).catch(() => null);
    });
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(preloadProductData, 1000);
    });
} else {
    setTimeout(preloadProductData, 1000);
}
// Предзагрузка данных для всех товаров при загрузке страницы
async function preloadProductData() {
    const productItems = document.querySelectorAll('.product-item');
    const productNames = Array.from(productItems)
        .map(item => item.querySelector('.product-name')?.textContent?.trim())
    ...
    // Загружаем данные для первых 5 товаров
    const uniqueNames = [...new Set(productNames)].slice(0, 5);
    
    for (const name of uniqueNames) {
        try {
            await fetch(`${API_BASE_URL}/catalog/short/info?name=${encodeURIComponent(name)}`);
        } catch (error) {
            console.log(`Предзагрузка для ${name} не удалась`);
        }
    }
}
